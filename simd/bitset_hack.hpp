...
namespace hack _GLIBCXX_VISIBILITY(default)
...
_Base_bitset<_Nw>::_M_do_to_ulong() const
{
  for (size_t __i = 1; __i < _Nw; ++__i)
    if (_M_w[__i])
      std::__throw_overflow_error(__N("_Base_bitset::_M_do_to_ulong"));
  return _M_w[0];
}
...
_Base_bitset<_Nw>::_M_do_to_ullong() const
{
  ...
  for (size_t __i = 1 + __dw; __i < _Nw; ++__i)
    if (_M_w[__i])
      std::__throw_overflow_error(__N("_Base_bitset::_M_do_to_ullong"));
...
struct _Base_bitset<0>
{
  ...
  _M_getword(size_t) _GLIBCXX_NOEXCEPT
  {
    std::__throw_out_of_range(__N("_Base_bitset::_M_getword"));
    return *new _WordT;
  }
...
private:
                            ...
                            size_t __position) const
  {
    if (__position > __s.size())
      std::__throw_out_of_range_fmt(__N("bitset::bitset: __position "
  ...
  {
    if (__position >= _Nb)
      std::__throw_out_of_range_fmt(__N("%s: __position (which is %zu) "
                                       ">= _Nb (which is %zu)"),
                               __s, __position, _Nb);
  }
...
public:
  ...
  {
    if (!__str)
      std::__throw_logic_error(__N("bitset::bitset(const _CharT*, ...)"));
           ...
           const bitset<_Nb>& __x)
{
  ...
  // _GLIBCXX_RESOLVE_LIB_DEFECTS
  // 396. what are characters zero and one.
  const std::ctype<_CharT>& __ct = std::use_facet<std::ctype<_CharT> >(__os.getloc());
...
//template<size_t _Nb>
//struct std::hash<_GLIBCXX_STD_C::bitset<_Nb>>
//    : public std::__hash_base<size_t, _GLIBCXX_STD_C::bitset<_Nb>>
//{
//  size_t
