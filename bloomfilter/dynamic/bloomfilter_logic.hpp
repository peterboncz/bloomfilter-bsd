...
#include "bloomfilter_block_logic.hpp"
...
//  static_assert(hash_value_bitlength == block_t::hash_value_bitlength, "Hash value bitlength mismatch.");
  ...
  std::unique_ptr<bloomfilter_addressing_logic> addr;
  /// The block logic.
  bloomfilter_block_logic<word_t, key_t, HashFn> block;
  //===----------------------------------------------------------------------===//
  ...
  //===----------------------------------------------------------------------===//
  static std::unique_ptr<bloomfilter_addressing_logic>
 ...
 public:
  ...
  explicit
  bloomfilter_logic(const std::size_t length /* in bits */,
                    u32 block_size_bytes,
  ...
  bloomfilter_logic(const bloomfilter_logic& src) noexcept
      : addr(src.addr.get()),
        block(src.block) {}

  bloomfilter_logic(bloomfilter_logic&& src) noexcept
      : addr(std::move(src.addr)),
        block(std::move(src.block)) {}

  bloomfilter_logic& operator=(bloomfilter_logic&& src) {
    addr = std::move(src.addr);
    block = std::move(src.block);
  };
