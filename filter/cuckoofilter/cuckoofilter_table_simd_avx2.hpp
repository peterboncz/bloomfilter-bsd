...
struct find_tag_in_buckets_simd<8, 2> {
  ...
  dispatch(const uint32_t* __restrict filter_data,
    ...
    const auto t1 = reinterpret_cast<const r256*>(&tag1.data);
    const auto t2 = reinterpret_cast<const r256*>(&tag2.data);
    const auto st_u8 = reinterpret_cast<const r256*>(&search_tag_u8.data);
    auto fm = reinterpret_cast<r256*>(&found_mask.data);

    const r256 msb_off = {.i = _mm256_set1_epi32( uint32_t(-1) >> 1 )};
    const auto zero = _mm256_setzero_si256();

    using vector_type = decltype(tag1);
    u64 vector_cnt = sizeof(vector_type) / sizeof(r256);

    for (std::size_t i = 0; i < vector_cnt; i++) {
      const r256 t1_m = {.i = _mm256_cmpeq_epi8(t1[i].i, st_u8[i].i) };
      const r256 t2_m = {.i = _mm256_cmpeq_epi8(t2[i].i, st_u8[i].i) };
      const r256 found1_m = {.i = _mm256_cmpgt_epi32(_mm256_and_si256(t1_m.i, msb_off.i), zero) }; // turn off msb of mask for signed comparison
      const r256 found2_m = {.i = _mm256_cmpgt_epi32(_mm256_and_si256(t2_m.i, msb_off.i), zero) };

      // distinguish between AVX2 and AVX-512 masks
      fm[i].i = _mm256_or_si256(found1_m.i, found2_m.i);
    }
    return found_mask;
  }
...
struct find_tag_in_buckets_simd<8, 4> {
  ...
  dispatch(const uint32_t* __restrict filter_data,
    ...
    const r256 msb_off = {.i = _mm256_set1_epi32( uint32_t(-1) >> 1 )};
    const auto zero = _mm256_setzero_si256();
    for (std::size_t i = 0; i < tag1.nested_vector_cnt; i++) {
      const r256 t1_m = {.i = _mm256_cmpeq_epi8(t1[i].i, st_u8[i].i) };
      const r256 t2_m = {.i = _mm256_cmpeq_epi8(t2[i].i, st_u8[i].i) };
      const r256 found1_m = {.i = _mm256_cmpgt_epi32(_mm256_and_si256(t1_m.i, msb_off.i), zero) }; // turn off msb of mask for signed comparison
      const r256 found2_m = {.i = _mm256_cmpgt_epi32(_mm256_and_si256(t2_m.i, msb_off.i), zero) };
...
struct find_tag_in_buckets_simd<16, 2> {
  ...
  dispatch(const uint32_t* __restrict filter_data,
    ...
    const auto t1 = reinterpret_cast<const r256*>(&tag1.data);
    const auto t2 = reinterpret_cast<const r256*>(&tag2.data);
    const auto st_u16 = reinterpret_cast<const r256*>(&search_tag_u16.data);
    auto fm = reinterpret_cast<r256*>(&found_mask.data);

    const r256 msb_off = {.i = _mm256_set1_epi32( uint32_t(-1) >> 1 )};
    const auto zero = _mm256_setzero_si256();
    for (std::size_t i = 0; i < tag1.nested_vector_cnt; i++) {
      const r256 t1_m = {.i = _mm256_cmpeq_epi16(t1[i].i, st_u16[i].i) };
      const r256 t2_m = {.i = _mm256_cmpeq_epi16(t2[i].i, st_u16[i].i) };
      const r256 found1_m = {.i = _mm256_cmpgt_epi32(_mm256_and_si256(t1_m.i, msb_off.i), zero) }; // turn off msb of mask for signed comparison
      const r256 found2_m = {.i = _mm256_cmpgt_epi32(_mm256_and_si256(t2_m.i, msb_off.i), zero) };
      fm[i].i = _mm256_or_si256(found1_m.i, found2_m.i);
    }
...
struct find_tag_in_buckets_simd<16, 4> {
  ...
  dispatch(const uint32_t* __restrict filter_data,
    ...
    const auto t1a = reinterpret_cast<const r256*>(&tag1a.data);
    const auto t1b = reinterpret_cast<const r256*>(&tag1b.data);
    const auto t2a = reinterpret_cast<const r256*>(&tag2a.data);
    const auto t2b = reinterpret_cast<const r256*>(&tag2b.data);
    const auto st_u16 = reinterpret_cast<const r256*>(&search_tag_u16.data);
    auto fm = reinterpret_cast<r256*>(&found_mask.data);

    const r256 msb_off = {.i = _mm256_set1_epi32( uint32_t(-1) >> 1 )};
    const auto zero = _mm256_setzero_si256();
    for (std::size_t i = 0; i < tag1a.nested_vector_cnt; i++) {

      const r256 t1a_m = {.i = _mm256_cmpeq_epi16(t1a[i].i, st_u16[i].i) };
      const r256 t1b_m = {.i = _mm256_cmpeq_epi16(t1b[i].i, st_u16[i].i) };
      const r256 t2a_m = {.i = _mm256_cmpeq_epi16(t2a[i].i, st_u16[i].i) };
      const r256 t2b_m = {.i = _mm256_cmpeq_epi16(t2b[i].i, st_u16[i].i) };
      const r256 found1a_m = {.i = _mm256_cmpgt_epi32(_mm256_and_si256(t1a_m.i, msb_off.i), zero) }; // turn off msb of mask for signed comparison
      const r256 found1b_m = {.i = _mm256_cmpgt_epi32(_mm256_and_si256(t1b_m.i, msb_off.i), zero) };
      const r256 found1_m = {.i = _mm256_or_si256(found1a_m.i, found1b_m.i) };
      const r256 found2a_m = {.i = _mm256_cmpgt_epi32(_mm256_and_si256(t2a_m.i, msb_off.i), zero) };
      const r256 found2b_m = {.i = _mm256_cmpgt_epi32(_mm256_and_si256(t2b_m.i, msb_off.i), zero) };
      const r256 found2_m = {.i = _mm256_or_si256(found2a_m.i, found2b_m.i) };
      fm[i].i = _mm256_or_si256(found1_m.i, found2_m.i);
    }
    return found_mask;
  }
