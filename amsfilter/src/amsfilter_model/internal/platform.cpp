...
//===----------------------------------------------------------------------===//
platform::platform() :
    thread_cnt(std::max(u64(1), cpu_mask.count())),
  ...
  auto thread_fn = [&](u32 thread_id) {
    ...
    const auto mem_node_id =
        dtl::mem::hbm_available() ? dtl::mem::get_nearest_hbm_node(cpu_node_id)
                                  : cpu_node_id;
...
platform::get_memory_node_of_thread(u32 thread_id) const {
  const auto node_id = 
      thread_id_to_node_id_map[thread_id % thread_id_to_node_id_map.size()];
  return node_id;
}
//===----------------------------------------------------------------------===//
u32
...
platform::get_cache_sizes() const {
  ...
  while(true) {
    ...
    const std::string type_filename = 
        sys_path + "/index" + std::to_string(idx) + "/type";
    const std::string size_filename = 
        sys_path + "/index" + std::to_string(idx) + "/size";
